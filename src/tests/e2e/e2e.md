...
test.describe("Aplicação de Tarefas - E2E", () => {
  ...
  test("deve adicionar uma tarefa simples", async ({ page }) => {
    ...
    await expect(page.locator("li").last()).toContainText("Estudar JavaScript");
  });
  ...
  test("deve adicionar tarefa com caracteres especiais", async ({ page }) => {
    ...
    await expect(page.locator("li").last()).toContainText(specialTitle);
  });
  ...
  test("deve adicionar tarefa com espaços", async ({ page }) => {
    ...
    await expect(page.locator("li").last()).toContainText(titleWithSpaces);
  });
  ...
  test("deve limpar o campo de input após adicionar tarefa", async ({
    page,
  }) => {
    const input = page.locator("#title");
  ...
  test("deve manter tarefas após recarregamento da página", async ({
    page,
  }) => {
  ...
  test("deve adicionar tarefa com números", async ({ page }) => {
    ...
    await expect(page.locator("li").last()).toContainText(titleWithNumbers);
  });
  ...
  test("deve aceitar tarefas com emojis", async ({ page }) => {
    ...
    await expect(page.locator("li").last()).toContainText(titleWithEmoji);
  });

  test("deve existir um elemento de lista para as tarefas", async ({
    page,
  }) => {
  ...
  test("deve adicionar tarefa com texto longo", async ({ page }) => {
    const longTitle =
      "Esta é uma tarefa muito longa com muitos caracteres para testar se o sistema consegue lidar com títulos extensos sem problema algum";

    await page.fill("#title", longTitle);
    await page.click("button");

    await expect(page.locator("li").last()).toContainText(longTitle);
  });
  ...
  test("deve preservar maiúsculas e minúsculas", async ({ page }) => {
    ...
    await expect(page.locator("li").last()).toContainText(mixedCaseTitle);
  });
});
